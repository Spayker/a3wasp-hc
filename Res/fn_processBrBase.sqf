...
_unit_vehicles = [];
_unitTemplates = [
    [Format ["Team_AT_%1", floor random 3]],
    [Format ["Team_%1", floor random 3]]
];
...
WF_Logic setVariable [Format["%1%2TeamBaseInfantryPatrol%3",_sideText, _facIndex, 1], false];
WF_Logic setVariable [Format["%1%2TeamBaseInfantryPatrol%3",_sideText, _facIndex, 2], false];

while { alive _building } do {
    _patrolTeamAlive = WF_Logic getVariable Format ["%1%2TeamBaseInfantryPatrol%3", _sideText, _facIndex, 1];
	if (!_patrolTeamAlive) then {
        _ranTemp = _lightTemplate # (random((count _lightTemplate)-1));
        _templateToUse = _resTemplates # _ranTemp;
        [_side, _templateToUse, _building, Format ["%1%2TeamBaseInfantryPatrol%3", _sideText, _facIndex, 1], true] spawn WFHC_FNC_processResTeam;
        WF_Logic setVariable [Format ["%1%2TeamBaseInfantryPatrol%3", _sideText, _facIndex, 1], true]
	};
	_patrolTeamAlive = WF_Logic getVariable Format["%1%2TeamBaseInfantryPatrol%3", _sideText, _facIndex, 2];
	if (!_patrolTeamAlive) then {
        _ranTemp = _lightTemplate # (random((count _lightTemplate)-1));
        _templateToUse = _resTemplates # _ranTemp;
        [_side, _templateToUse, _building, Format ["%1%2TeamBaseInfantryPatrol%3", _sideText, _facIndex, 2], true] spawn WFHC_FNC_processResTeam;
        WF_Logic setVariable [Format["%1%2TeamBaseInfantryPatrol%3",_sideText, _facIndex, 2], true]
	};
	sleep 300
}