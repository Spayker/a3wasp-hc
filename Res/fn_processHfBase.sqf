...
_unit_vehicles = [];
_unitTemplates = [
    [Format ["Mechanized_%1", floor random 3]],
    [Format ["Armored_%1", floor random 3]]
];
...
while{alive _building}do{
	...
	if (!_patrolTeamAlive) then {
        _ranTemp = _heavyTemplate # (random((count _heavyTemplate)-1));
        _templateToUse = _resTemplates # _ranTemp;
        [_side,_templateToUse,_building,Format["%1%2TeamBaseHeavyPatrol%3",_sideText, _facIndex, 1]] spawn WFHC_FNC_processResTeam;
